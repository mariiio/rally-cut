...
VIDEO_PATH_MATCH3 = FIXTURES_DIR / "match-3-first-2min.MOV"
...
@pytest.mark.slow
class TestDetectionQualityMatch3:
    """Test rally detection quality against ground truth for match 3."""

    @pytest.fixture
    def expected_rallies(self) -> list[tuple[float, float]]:
        """Load expected rallies from ground truth file."""
        return get_rally_times("match-3-first-2min.MOV")

    @pytest.fixture
    def detected_segments(self) -> list[tuple[float, float]]:
        """Run detection pipeline and return detected rally times."""
        if not VIDEO_PATH_MATCH3.exists():
            pytest.skip(f"Test video not found: {VIDEO_PATH_MATCH3}")

        # Use VideoCutter.analyze_only() with default config
        cutter = VideoCutter()

        segments = cutter.analyze_only(VIDEO_PATH_MATCH3)
        return [(seg.start_time, seg.end_time) for seg in segments]
