...
- **Project C additions** (shipped 2026-04-15):
  ...
  - Component B: silent auto-rotate during the optimize pass (both local + Lambda) when `|tiltDeg| > 5° AND linesScored >= 3` (`shouldAutoRotate` in `processingService.ts`; the original beach-trained court-keypoint gate was replaced by Hough-line tilt detection because the keypoint model's confidence collapsed at >10° rotation — see commit `a7018611`). Auto-rotate also fires when `wantsRotate=true` even if `checkNeedsOptimization=false` (`shouldSkipOptimization` gates on both signals; closes a 2026-05-13 bypass). Adds `qualityReportJson.autoFixes[]` — a separate field from `issues[]` — logging "auto-straightened by N°". Tilt detection runs synchronously inside `generatePosterImmediate`, and `confirmVideoUpload` sequences `generatePosterImmediate().then(queueVideoProcessing)` so the rotate decision always reads the freshly-written tilt fields (fix for the 2026-05-13 race that left juju.mp4 unrotated at -9°).
...
- **Batch tracking**: `POST /v1/videos/:id/track-all-rallies` tracks all confirmed rallies
  ...
  - Batch completion (Modal webhook OR local-CPU `scripts/batchTrack.ts` worker) updates `BatchTrackingJob.status='COMPLETED'`; match analysis is triggered client-side after a 5-second idle window (Project A2a). Both paths are now uniform — neither auto-triggers `runMatchAnalysis` server-side. The client's `pollTracking` observes completion, the debounce arms catch-up tracking for any rallies created mid-batch, then fires the SSE match-analysis request.
  ...
  - `POST /v1/videos/:id/run-match-analysis` (A2a, client-initiated): SSE endpoint that streams stage progress and resolves only when every stage truly completes. Gated by the same in-memory `runningVideos` lock as the trigger endpoint — returns 409 with `details.reason='MATCH_ANALYSIS_IN_PROGRESS'` if another tab is mid-run; client falls back to polling `/match-analysis-status`. This is the path the editor's "Analyze Match" / "Retrack & analyze" / "Re-analyze" buttons take.
  - `POST /v1/videos/:id/trigger-match-analysis` (ops / scripts): fire-and-forget variant. Returns 202 immediately, 409 if already running. No longer called by the web client.
  - `GET /v1/videos/:id/match-analysis-status`: lifecycle for the pipeline derived from `Video.matchAnalysisStartedAt` / `matchAnalysisRanAt` / `matchAnalysisError`. Returns `{ status: 'idle' | 'processing' | 'completed' | 'failed', startedAt, ranAt, error }`. Used by the editor to detect true completion (instead of inferring from `matchStatsJson` presence, which is stale across re-analyses) and to resume after page reload during a long run.
  - **Match analysis pipeline**: validate-rallies → match-players → repair-identities → remap-track-ids → reattribute-actions → compute-match-stats (all best-effort, non-fatal). `runMatchAnalysis` writes `matchAnalysisStartedAt` at the top and `matchAnalysisRanAt` + `matchAnalysisError` in a `finally`, so completion is always observable via the status endpoint regardless of which stage succeeded.
...
- **Delete**: `DELETE /v1/rallies/:id` deletes the rally and prunes its entry from `matchAnalysisJson.rallies[]` so stats don't reference a dead rally. Cascades the `PlayerTrack`.
