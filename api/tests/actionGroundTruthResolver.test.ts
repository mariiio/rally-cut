...
type Pos = { trackId: number; bbox: { x1: number; y1: number; x2: number; y2: number }; embedding: Float32Array | null };

const at = (trackId: number, x1: number, y1: number, x2: number, y2: number, embedding: Float32Array | null = null): Pos => ({
  trackId, bbox: { x1, y1, x2, y2 }, embedding,
});
...
describe('resolveGtRow', () => {
  it('returns SNAPSHOT_EXACT when bbox is null but snapshotTrackId is present at frame', () => {
    const row = { snapshotBboxX1: null, snapshotBboxY1: null, snapshotBboxX2: null, snapshotBboxY2: null, snapshotTrackId: 7, snapshotReidEmbedding: null };
  ...
  it('returns UNRESOLVED when bbox null and trackId absent from frame', () => {
    const row = { snapshotBboxX1: null, snapshotBboxY1: null, snapshotBboxX2: null, snapshotBboxY2: null, snapshotTrackId: 9, snapshotReidEmbedding: null };
    const positions = [at(7, 0.1, 0.1, 0.2, 0.3)];
  ...
  it('returns IOU_MATCH when bbox overlaps >= 0.5 with a candidate', () => {
    const row = { snapshotBboxX1: 0.1, snapshotBboxY1: 0.1, snapshotBboxX2: 0.2, snapshotBboxY2: 0.3, snapshotTrackId: 99, snapshotReidEmbedding: null };
  ...
  it('returns NEAREST_CENTER when IoU is low but center within 0.10', () => {
    const row = { snapshotBboxX1: 0.10, snapshotBboxY1: 0.10, snapshotBboxX2: 0.12, snapshotBboxY2: 0.14, snapshotTrackId: 99, snapshotReidEmbedding: null };
  ...
  it('returns UNRESOLVED when no candidate meets either threshold', () => {
    const row = { snapshotBboxX1: 0.10, snapshotBboxY1: 0.10, snapshotBboxX2: 0.20, snapshotBboxY2: 0.30, snapshotTrackId: 99, snapshotReidEmbedding: null };
  ...
  it('returns UNRESOLVED when positions are empty and bbox is present', () => {
    const row = { snapshotBboxX1: 0.1, snapshotBboxY1: 0.1, snapshotBboxX2: 0.2, snapshotBboxY2: 0.3, snapshotTrackId: 7, snapshotReidEmbedding: null };
    expect(resolveGtRow(row, [])).toEqual({ resolvedTrackId: null, resolvedSource: 'UNRESOLVED' });
  });
  ...
  it('returns REID_MATCH when IoU fails but embedding cosine >= 0.5', () => {
    ...
    const row = {
      ...
      snapshotTrackId: 99, snapshotReidEmbedding: snapshotEmb,
    };
  ...
  it('REID tier is skipped when snapshot embedding is null', () => {
    const row = {
      ...
      snapshotTrackId: 99, snapshotReidEmbedding: null,
    };
  ...
  it('IoU tier still wins when both IoU and REID would match', () => {
    ...
    const row = {
      snapshotBboxX1: 0.1, snapshotBboxY1: 0.1, snapshotBboxX2: 0.2, snapshotBboxY2: 0.3,
      snapshotTrackId: 99, snapshotReidEmbedding: emb,
    };
