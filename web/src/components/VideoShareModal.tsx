...
export function VideoShareModal({ open, onClose, videoId, videoName, isOwner }: VideoShareModalProps) {
  ...
  const loadOrCreateShare = useCallback(async () => {
    try {
      ...
      let share = await getVideoShare(videoId);

      if (!share || share.shares.length === 0) {
        const created = await createVideoShare(videoId);
        share = {
  ...
  const handleRemoveMember = async (userId: string) => {
    try {
      ...
      await removeVideoShareMember(videoId, userId);
      const updated = await getVideoShare(videoId);
  ...
  const handleDeleteShare = async () => {
    try {
      setConfirmingDelete(false);
      setDeleting(true);
      await deleteVideoShare(videoId);
      onClose();
    } catch (err) {
  ...
  const handleMemberRoleChange = async (userId: string, newRole: MemberRole) => {
    try {
      ...
      await updateVideoMemberRole(videoId, userId, newRole);
      setShareInfo((prev) => {
        if (!prev) return prev;
        return {
          ...prev,
