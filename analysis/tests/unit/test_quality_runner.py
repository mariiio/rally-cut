...
def test_runner_produces_serializable_report_with_good_video(tmp_path):
    ...
    with patch("rallycut.quality.runner._load_video_inputs", return_value=(meta, frames, corners, [[], [], []], (0.2, 0.3, 0.8, 0.9))):
...
def test_runner_surfaces_block_issue_for_bad_video(tmp_path):
    ...
    with patch("rallycut.quality.runner._load_video_inputs", return_value=(meta, frames, bad_corners, [[], [], []], (0.2, 0.3, 0.8, 0.9))):
        report = run_full_preflight("/tmp/fake.mp4", sample_seconds=60)
