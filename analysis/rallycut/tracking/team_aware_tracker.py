...
class TeamSideTracker:
    ...
    def _check_separation(self) -> bool:
        ...
        logger.debug(
            ...
            near_median, far_median, separation, self.config.min_team_separation,
        )
        ...
        min_c = self.config.min_clearance_from_split

        if near_clearance < min_c:
            logger.debug(
                "Team-aware: near team too close to split "
                "(clearance=%.3f < %.3f), staying inactive",
                near_clearance, min_c,
            )
            return False
        if far_clearance < min_c:
            logger.debug(
    ...
    def compute_penalty_matrix(
        ...
        if not self._active or n_tracks == 0 or n_dets == 0:
            return penalty
        ...
        for i, track in enumerate(tracks):
            ...
            for j, det in enumerate(detections):
                # Detection Y: center of bbox in normalized coords
                # STrack stores tlwh (top-left, width, height) in pixels
                tlwh = det.tlwh
                det_cy = (tlwh[1] + tlwh[3] / 2) / img_height

                # Signed distance from expected side boundary
                if team == 0:  # near team: expected y > court_split_y
                    signed_dist = self.court_split_y - det_cy
                else:  # far team: expected y < court_split_y
                    signed_dist = det_cy - self.court_split_y

                # Dead zone: no penalty within net_band of court_split
