...
def main() -> None:
    ...
    for rally in rallies:
        ...
        if cached is not None:
            vballnet_preds = apply_ball_filter_config(
                cached.raw_ball_positions, filter_config
            )
        ...
        console.print(
            ...
            f"({inference_time:.1f}s, {len(tracknet_preds)} raw → "
            f"{len(tracknet_filtered)} filtered)"
        )
        print()
    ...
    for (rid, vr), (_, tr) in zip(vballnet_results, tracknet_results):
        ...
        if vr["match"] > tr["match"]:
            winner = "[blue]VballNet[/blue]"
        elif tr["match"] > vr["match"]:
