...
def render_report(agg: Aggregates) -> str:
    header = [
        ...
        "`reports/tracking_audit/*.json` (built by",
        "`rallycut evaluate-tracking --all --retrack --cached --audit-out reports/tracking_audit`,",
        "which reflects true pipeline output — NOT `player_tracks.positions_json`,",
        "which has been manually patched in the live DB).",
        "",
        "## Headline",
        "",
    ...
    header.extend([
        ...
        "## Next steps",
        "",
        "1. Pick one cause from the recommendations above, scope a follow-up plan with `/brainstorm`.",
        "2. After any pipeline change, regenerate via:",
        "   `rallycut evaluate-tracking --all --retrack --cached --audit-out reports/tracking_audit`",
        "   then `uv run python scripts/write_tracking_audit_report.py`.",
        "3. (Optional) Rebuild the HTML gallery via `scripts/build_tracking_audit_gallery.py` if visual triage is needed.",
        "",
    ])
    return "\n".join(header)
